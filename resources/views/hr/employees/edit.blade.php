...
<div class="bg-white rounded-lg shadow-md p-6">
    <form method="POST" action="{{ route('hr.employees.update', $employee) }}">
        ...
        <div class="mb-6">
            ...
            <input type="number" name="salary_rate" id="salary_rate" value="{{ old('salary_rate', $employee->salary_rate) }}" step="0.01" min="0" required 
        ...
        <div class="mb-6">
            ...
            <textarea name="notes" id="notes" rows="3" 
                      class="w-full border rounded-lg px-4 py-2 @error('notes') border-red-500 @enderror">{{ old('notes', $employee->notes) }}</textarea>
            @error('notes')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
